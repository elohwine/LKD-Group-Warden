...
export async function removeQueueItemById(id) {
  if (!id) return false;
...
export function createSerialTaskQueue() {
  ...
  const enqueue = (task, key = '__global__') => {
    const dedupeKey = String(key ?? '__global__');
    ...
    inFlightByKey.set(dedupeKey, run);

    chain = run.catch(() => undefined).finally(() => {
      if (inFlightByKey.get(dedupeKey) === run) {
