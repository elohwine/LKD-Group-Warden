...
function normalizeCurrentUkMiddleDigits(value) {
  const input = String(value || '');
  if (input.length !== 7) return input;

  const chars = input.split('');
  // Current UK format LLDDLLL uses the middle pair (index 2 and 3) as digits.
  for (const index of [2, 3]) {
    chars[index] = OCR_TO_DIGIT[chars[index]] || chars[index];
  }

  return chars.join('');
}

function normalizeAmbiguousCharacter(expectedType, value) {
  if (!value) return value;

  if (expectedType === 'D') {
    if (/^[0-9]$/.test(value)) return value;
    return OCR_TO_DIGIT[value] || value;
  }
...
function buildForMask(input, mask) {
  ...
  for (let i = 0; i < mask.length; i += 1) {
    ...
    const value = chars[i];
    const replacement = normalizeAmbiguousCharacter(expected, value);

    if (value === replacement) continue;
    if (!/^[A-Z0-9]$/.test(replacement)) return null;
...
export function scoreUkVrmCandidate(value) {
  const clean = normalizeCurrentUkMiddleDigits(sanitize(value));
  if (!clean) return 0;
