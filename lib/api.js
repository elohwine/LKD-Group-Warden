...
// In browser dev mode, use relative URLs so local Next.js handles /api/* routes
// without triggering a CORS block against the production backend.
const IS_BROWSER_DEV = typeof window !== 'undefined' && process.env.NODE_ENV === 'development';

export function buildApiUrl(path) {
  if (!path) return DEFAULT_API_BASE || '';
  if (/^https?:\/\//i.test(path)) return path;
  const normalizedPath = path.startsWith('/') ? path : `/${path}`;
  // In local dev, use relative paths so /api/* is served by the local Next.js
  // server — avoids CORS errors when hitting production endpoints cross-origin.
  if (IS_BROWSER_DEV) return normalizedPath;
